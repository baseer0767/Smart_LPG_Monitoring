...
// // Server to POST data
// const char* serverURL = "http://172.16.112.113:3000/api/loadcell/device01";
...
// doc["weight"] = weight;                 // total weight in kg
// doc["capacityKg"] = maximum_weight_kg;  // cylinder capacity
// doc["availableGasKg"] = weight;         // available gas (same as weight for now)
// doc["lastRefill"] = "2025-09-17";       // example date (optional)
...
const int LOADCELL_DOUT_PIN = D2;
const int LOADCELL_SCK_PIN = D1;
const int PWM_OUTPUT_PIN    = D5; // GPIO14
...
// Server to POST data
//const char* serverURL = "http://172.16.112.147:3000/api/loadcell/checking";
//const char* serverURL = "http://172.16.112.123:3000/api/loadcell/try";
const char* serverURL = "https://smart-lpg-tracker-production.up.railway.app//api/loadcell/local";
...
void loop() {
  ...
  const float threshold = 0.05; // 50 grams

  if (WiFi.status() == WL_CONNECTED) {
    float weight = scale.get_units(10) / 1000.0; // kg
    if (weight < 0) weight = 0;

    if (abs(weight - lastWeight) >= threshold) {
      lastWeight = weight;

      // Optionally control PWM as an indicator
      int pwmValue = map(weight, 0, 60, 0, 255); // still mapping up to 60kg
      pwmValue = constrain(pwmValue, 0, 255);
      analogWrite(PWM_OUTPUT_PIN, pwmValue);

      // Round weight to 6 decimals
      weight = round(weight * 1000000) / 1000000.0;

      // Debug print
      Serial.printf("Total Weight: %.6f kg\n", weight);

      // Send only raw weight to server
      HTTPClient https;
      ...
      StaticJsonDocument<200> doc;
      //doc["deviceId"] = "checking";
      doc["totalWeightKg"] = weight;
      ...
      if (response > 0) {
        ...
        Serial.printf("Error sending data: %d\n", response);
        Serial.println("Error details:");
        Serial.println(https.errorToString(response));
      }
